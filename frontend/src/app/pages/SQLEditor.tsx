...
import { Play, Save, BarChart3, Database, ChevronDown, Clock, Activity, AlertTriangle, Copy, Sparkles } from 'lucide-react';
...
LIMIT 100;`);
    ...
    const [applyRecommendations, setApplyRecommendations] = useState<boolean>(false);
    ...
    const loadConnections = async () => {
        try {
            ...
            setConnections(data);
            if (data.length > 0 && !selectedConnectionId) setSelectedConnectionId(data[0].id);
        } catch (error) {
    ...
    const handleExecute = async () => {
        if (!selectedConnectionId) { alert('Select connection'); return; }
        setExecuting(true); setActiveTab('results');
        try {
            const { data } = await analyticsApi.execute({ connectionId: selectedConnectionId, query: sql, iterations });
            setQueryResult(data);
            if (data.success) {
                const connection = connections.find(c => c.id === selectedConnectionId);
                saveQueryToHistory({ query: sql, connectionId: selectedConnectionId, connectionName: connection?.name || 'Unknown', executionTimeMs: data.executionTimeMs, rowCount: data.rowCount });
            }
        } catch (error: any) {
            console.error('Execution failed', error);
            setQueryResult({ success: false, errorMessage: error.response?.data?.message || 'Query execution failed', columns: [], rows: [], rowCount: 0, executionTimeMs: 0 });
        } finally { setExecuting(false); }
    };

    const handleAnalyze = async () => {
        if (!selectedConnectionId) { alert('Select connection'); return; }
        setExecuting(true); setActiveTab('analysis');
        try {
            const { data } = await analyticsApi.analyze({ connectionId: selectedConnectionId, sqlQuery: sql, autoApply: false, iterations, applyRecommendations });
    ...
    const copySuggestion = (suggestion: string) => navigator.clipboard.writeText(suggestion);
    const getConnectionName = () => connections.find(c => c.id === selectedConnectionId)?.name || 'Выберите подключение';

    const sortedRecommendations = analysisData?.recommendations
        ? [...analysisData.recommendations].sort((a, b) => {
            const impactOrder: Record<string, number> = { Высокий: 4, Средний: 3, Низкий: 2, Информационный: 1 };
            const impactA = impactOrder[a.impact] || 0;
            const impactB = impactOrder[b.impact] || 0;
            if (impactA !== impactB) return impactB - impactA;
            const improvementA = a.estimatedImprovement || 0;
            const improvementB = b.estimatedImprovement || 0;
    ...
    return (
        <div className="h-screen flex flex-col" style={{ backgroundColor: 'var(--pg-bg-primary)' }}>
            <div className="flex items-center justify-between px-4 py-3 border-b" style={{ backgroundColor: 'var(--pg-bg-surface)', borderColor: 'var(--pg-border)' }}>
                <div className="flex items-center gap-3">
                    <div className="relative">
                        <button onClick={() => setShowConnectionDropdown(!showConnectionDropdown)} className="flex items-center gap-2 px-3 py-2 rounded-lg border transition-colors" style={{ backgroundColor: 'var(--pg-bg-card)', borderColor: 'var(--pg-border)', color: 'var(--pg-text-primary)' }} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--pg-border-light)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--pg-border)'}>
                        ...
                        {showConnectionDropdown && (
                            <div className="absolute top-full left-0 mt-1 w-64 rounded-lg border shadow-lg overflow-hidden z-10" style={{ backgroundColor: 'var(--pg-bg-card)', borderColor: 'var(--pg-border)' }}>
                                {connections.map(conn => (
                                    <button key={conn.id} onClick={() => { setSelectedConnectionId(conn.id); setShowConnectionDropdown(false); }} className="w-full px-3 py-2.5 text-left text-sm transition-colors" style={{ backgroundColor: conn.id === selectedConnectionId ? 'var(--pg-bg-hover)' : 'transparent', color: 'var(--pg-text-primary)' }} onMouseEnter={e => e.currentTarget.style.backgroundColor = 'var(--pg-bg-hover)'} onMouseLeave={e => e.currentTarget.style.backgroundColor = conn.id === selectedConnectionId ? 'var(--pg-bg-hover)' : 'transparent'}>
                                        {conn.name}
                                    </button>
                                ))}
                            </div>
                ...
                <Split className="flex h-full" sizes={[50, 50]} minSize={300} gutterSize={8} gutterStyle={() => ({ backgroundColor: 'var(--pg-border)', cursor: 'col-resize' })}>
                    ...
                    <div className="flex flex-col h-full">
                        ...
                        <div className="flex-1 overflow-auto" style={{ backgroundColor: 'var(--pg-bg-surface)' }}>
                            {activeTab === 'results' ? (
                                <div className="p-4">
                                    {queryResult && !queryResult.success && <div className="mb-4 p-3 rounded bg-red-500/20 text-red-400 text-sm">{queryResult.errorMessage}</div>}
                                    {queryResult?.success && (
                                        <>
                                                ...
                                                <div className="rounded-lg border overflow-hidden" style={{ borderColor: 'var(--pg-border)' }}>
                                                    <div className="overflow-x-auto">
                                                        <table className="w-full text-sm">
                                                            <thead><tr style={{ backgroundColor: 'var(--pg-bg-card)' }}>{queryResult.columns.map((col, idx) => <th key={idx} className="px-4 py-3 text-left font-medium text-xs" style={{ color: 'var(--pg-text-primary)' }}>{col}</th>)}</tr></thead>
                                                            <tbody>{queryResult.rows.map((row, i) => <tr key={i} className="border-t" style={{ borderColor: 'var(--pg-border)', backgroundColor: i % 2 === 0 ? 'transparent' : 'var(--pg-bg-card)' }}>{row.map((cell, j) => <td key={j} className="px-4 py-2.5 font-mono text-xs" style={{ color: 'var(--pg-text-secondary)' }}>{cell !== null ? String(cell) : 'NULL'}</td>)}</tr>)}</tbody>
                                                        </table>
                                                    </div>
                                                </div>
                                ...
                                <div className="p-4 space-y-4">
                                    ...
                                    ) : <div className="text-center py-12" style={{ color: 'var(--pg-text-muted)' }}>{analysisData?.errorMessage || 'Press Analyze to get recommendations.'}</div>}
                                </div>
                            )}
                        </div>
                    </div>
